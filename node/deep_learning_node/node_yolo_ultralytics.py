...
class Node(DpgNodeABC):
    ...
    node_label = 'YOLO(ultralytics)'
    node_tag = 'YOLO_ultralytics'
    ...
    _model_class = {
        'Detect (COCO)':_model_base_path + '/object_detection/ultralytics/model/yolov8n.onnx',
        'Pose (COCO)':_model_base_path + '/pose_estimation/ultralytics_pose/model/yolov8n-pose.onnx',
    }
    ...
    def __init__(self):
        ...
        #modelsフォルダにあるファイルをモデル設定に追加
        file_list = os.listdir(self._model_base_path + '/models')
        for file_name in file_list:
            self._model_class.update({file_name:self._model_base_path + '/models/' + file_name})
    ...
    def update(
        ...
        model_path = self._model_class[model_name]
            ...
            if model_name_with_provider not in self._model_instance:
                if provider == 'CPU':
                    ...
                    self._model_instance[
                        model_name_with_provider] = YOLO(model_path)
                else:
                    self._model_instance[
                        model_name_with_provider] = YOLO(model_path)
        ...
        if frame is not None:
            results = self._model_instance[
                model_name_with_provider].predict(frame, conf=score_th, show=False)
