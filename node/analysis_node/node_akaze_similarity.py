...
def calculate_similarity(img1, img2):
    ...
    result_img = cv2.drawMatches(img1, img1_kp, img2, img2_kp, matches[:10], None, flags=2)
...
def image_process(images):
    img1 = images[0]
    img2 = images[1]
    image, results = calculate_similarity(img1, img2)
    return image, results

class Node(DpgNodeABC):
    _ver = '0.0.1'

    node_label = 'AKAZE Similarity'
    node_tag = 'AKAZE_Similarity'
    ...
    def update(
        ...
        if frames[0] is not None and frames[1] is not None:
            image, results = image_process(frames)
